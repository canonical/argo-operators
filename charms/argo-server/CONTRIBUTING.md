...
This document outlines the processes and practices recommended for contributing enhancements to `argo-server`.
...
# Deploy the charm
juju deploy ./argo-server_ubuntu-20.04-amd64.charm \
    --resource oci-image=$(yq '.containers."argo-server".resources."oci-image"."upstream-source"' metadata.yaml)
